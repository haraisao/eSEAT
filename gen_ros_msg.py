...
DEFAULT_INCLUDES = ['std_msgs', 'geometry_msgs', 'sensor_msgs']
...
def generate_messages(package, includes=DEFAULT_INCLUDES):
  ...
  for gentype in ('msg', 'srv'):
    ...
    if files:
      ...
      for include in set(includes+get_package_names()):
          search_path[include]=[os.path.join(LIB_DIR, include, 'msg') ,os.path.join(ROS_SHARE_DIR, include, 'msg')]
